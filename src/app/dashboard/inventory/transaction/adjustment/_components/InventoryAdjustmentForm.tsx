...
function InventoryAdjustmentForm({
    ...
    const [itemFormState, formSubmitAction] = useFormState(
        async (prevState: FormState, formData: FormData) => {
            const inventoryAdjustment = {} as IInventoryAdjustment;
            if (currentItem?.id) inventoryAdjustment.id = currentItem.id;
            ...
            [...formData.entries()].forEach((entry) => {
                ...
                if (!key.includes('$ACTION_ID_') && !['item_id', 'item_quantity'].includes(key))
                    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                    // @ts-ignore
                    inventoryAdjustment[key] = value;
            });

            const items: IInventoryAdjustmentItem[] = [];
            ...
            if (itemIds.length > 0 && itemIds.length === itemQuantities.length) {
                for (let i = 0; i < itemIds.length; i++) {
                    ...
                    } as IInventoryAdjustmentItem);
                }
            }

            if (items.length > 0) inventoryAdjustment.items = items;

            const currentFormState = await handleSubmit(inventoryAdjustment);
                ...
                {fields.map((field) => (
                    <div key={field.name} className={field.fullWidth ? 'col-span-2' : ''}>
                        <Input
                            field={{
                                ...field,
                                ...(currentItem?.[field.name as keyof IInventoryAdjustment]
                                    ? {
                                          defaultValue:
                                              currentItem[field.name as keyof IInventoryAdjustment],
                                      }
